...
abstract class parser {
	...
	function export($table_name='', $db_name='', $nodata = false,$nostruct = false) {
		...
		$dump = '';
		$dump .= "-- \n";
		$dump .= '-- MySQL DATABASE DUMPER. Copyright AnnaNovas &reg;\n\n '."\n";
		$dump .= "-- \n\n";
...
//					if ( stristr ($filed, 'file') && !empty($this->data[$filed]['name']) ){
//						$record[$filed] = $this->data[$filed]['name'];
//					}	
//					else if (!stristr ($filed, 'file')){
...
//					}
	...
	function executeReplace ($table, $field_array, $keyCols){
...
//					}
			...
			//Returns 0 on failure, 1 if update statement worked, 2 if no record was found and the insert was executed successfully
			$excution_type = getConfigValue('dbhandler')->db->Replace($table, $record,$keyCols,$autoquote = true);  
		 
			//getConfigValue('dbhandler')->db->AutoExecute($table, $record, 'INSERT');
			$this->error_string .= $this->getError();
			
			return $excution_type;//getConfigValue('dbhandler')->db->Insert_ID ();
		}
	...
	function executeEdit ($table, $field_array){
		...
		foreach ($columns_info as $column ){
			...
			if (  stristr($column['Type'], 'int') && empty ($this->data[$column['Field']])){
...
//					}
	...
	function renderFPagination ($base_url, $num_items, $per_page, $on_page, $add_prevnext_text = TRUE){
      ...
      <ul>';
		...
		{
			if ($i != 0)
				$page_string .= ( $i == $on_page ) ? '<li class="select">' . $i . '</li>' : '<li><a href="' .$base_url . "?&amp;current_page=" .$i.$paging_cotrolling_filed_url. '">' . $i . '</a></li><li></li>';	
		}
		...
		if ( $add_prevnext_text )
		{
			...
			if ( $on_page > 1 )
			{
				$page_string = '<li class="frist"><a href="'.$base_url ."?&amp;current_page=".( $on_page - 1 ).$paging_cotrolling_filed_url.'"><span>' .'Previous' . '</span></a></li><li></li>' . $page_string;
			}
			if ( $on_page < $total_pages )
			{
				$page_string .= '<li class="last"><a href="'.$base_url."?&amp;current_page=".( $on_page + 1 ).$paging_cotrolling_filed_url.'"><span>' . 'Next' . '</span></a></li>';
			}
		}
	...
	function renderPagination ($base_url, $num_items, $per_page, $on_page, $add_prevnext_text = TRUE){
		...
		{
			if ($i != 0)
				$page_string .= ( $i == $on_page ) ? '<li class="select-page"><span>' . $i . '</span></li>' : '<li><a href="' .$base_url . "?&amp;current_page=" .$i.$paging_cotrolling_filed_url. '"><span>' . $i . '</span></a></li><li></li>';	
		}
		$tmp = ($on_page < $total_pages)?$on_page+1:$on_page;
		...
		if ( $add_prevnext_text )
		{
			...
			if ( $on_page > 1 )
			{
				$page_string = '<li class="previous"><a href="'.$base_url ."?&amp;current_page=".( $on_page - 1 ).$paging_cotrolling_filed_url.'"><span>' .'Previous' . '</span></a></li><li></li>' . $page_string;
			}
			if ( $on_page < $total_pages )
			{
				$page_string .= '<li class="previous"><a href="'.$base_url."?&amp;current_page=".( $on_page + 1 ).$paging_cotrolling_filed_url.'"><span>' . 'Next' . '</span></a></li>';
			}
		}
	...
	function chkFAuthorization ($module_name){
			//echo '123';
			//echo $this->routing_data['lmscms_action'];
						
			$role_info = getSessionValue('fuser_role');
			$xml = io_file_get_contents(getConfigValue('module').$module_name."/Security.xml");
			$security_array =  xml_get_value($this->routing_data['lmscms_action'], $xml, 0);
			$security_role_array = xml_get_all_value ('role', $security_array['value'], 0);
