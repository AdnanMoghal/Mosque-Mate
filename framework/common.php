...
function getConfigValue ($index){
	//print_r($GLOBALS['cms_config'][$index]);
   // die;
...
function getJsAlt(){
	...
	if ($GLOBALS['cms_js']){
		foreach ($GLOBALS['cms_js'] as $js){
			 //echo $css;
			 //<link type="text/css" rel="stylesheet" href="css.css"/>
			 $js_html .= '<script src="'.$js.'" type="text/javascript" language="javascript"></script>';
		}
	}
...
function getReservedModuleList (){
	$reserved_module = array ('adminnavigation', 'adminpage', 'ajax', 'page', 'user','userrole');	
